...
class StudyGenerationProcess:
    ...
    def run(self):
        ...
        # 🔁 Only auto-generate if on step 1 AND hasn't been done yet
    ...
    def _auto_generate_full_study_product(self):
        ...
        with st.status("Generating study...", expanded=True) as status:
            ...
            status.update(label="🧪 Generating  Respondent Orientation...")
...
"""
            ...
            status.update(label="📋 Generating Final Thoughts...")
            final_thoughts_prompt = (
                ...
                "- Reflect the product’s tone (warm, human, innovative, or insightful)\n"
    ...
    def _step4_review(self):
        ...
        if st.button("💾 Run the study with 100 synthetic respondents"):
            ...
            if success:
                ...
                status_text.error("❌ Study creation failed")
...
class AgentsStudyGenerationProcess:
    def __init__(self, auth,agentName, df):
        self.auth = auth
        self.name= agentName.replace("Agent","")
        self.df=df
        self.data=""
    ...
    def run(self):
        self.data = self._process_df(self.df)
        if "agent_study_step" not in st.session_state:
            st.session_state.agent_study_step = 10
        if "agent_study_data" not in st.session_state or not st.session_state.agent_study_data:
            st.session_state.agent_study_data = {
                "study_name": "",
                "study_description": "",
                "questions": [],
                "prelim_questions": [],
        ...
        if "agent_study_autogenerated" not in st.session_state:
            st.session_state.agent_study_autogenerated = False
        # 🔁 Only auto-generate if on step 1 AND hasn't been done yet
        if st.session_state.agent_study_step == 0:
            with st.spinner(f"Thinking deeply about your {self.name}..."):
                self._auto_generate_full_study_product()
            st.session_state.agent_study_step = 4
            print("step updated to 4 ")
            st.session_state.agent_study_autogenerated = True
            st.rerun()
        # product_data = self.currentProduct

        if st.session_state.agent_study_step == 1:
            self._step1_setup(st.session_state.agent_study_data)
        elif st.session_state.agent_study_step == 2:
            self._step2_questions(st.session_state.agent_study_data)
        elif st.session_state.agent_study_step == 3:
            self._step3_prelim_questions()
        elif st.session_state.agent_study_step == 4:
            self._step4_review()
    ...
    def _auto_generate_full_study_product(self):
        ...
        with st.status("Generating study...", expanded=True) as status:
            status.update(label="🧠 Generating Study Description...")
            desc_prompt = (
                f"Create a comprehensive study description for  the {self.name}s  here is the list of {self.name} = {self.data} . "
                f""
                "Make this into a 4–5 paragraph response."
            )
            description = self._call_gpt(desc_prompt, max_tokens=400, json_mode=False)
            name_prompt=(
                f"Create a short name for a study about {description} . "
                "Make this into a 2-3 word response."
            )
            name=self._call_gpt(name_prompt, max_tokens=100, json_mode=False)
            status.update(label="🧪 Generating  Respondent Orientation...")
            respondent_orientation_prompt = (
                f"Write a short, friendly orientation message for a research study about {self.name}s  here is the list of {self.name} = {self.data}'.\n\n"
            ...
            status.update(label="🧪 Generating Main Questions...")
            q_prompt = (
                f"{self.name}s  here is the list of {self.name} = {self.data}"
                # Team Inputted Questions Prompt
                f"""
                We are interested in understanding the factors that make a person want to adopt or comply with a product, experience, or solution. These factors may include physical needs, emotional desires, lifestyle improvements, or other personal motivations.
                You are working with the following:
                {self.name}s  here is the list of {self.name} = {self.data}
                For this product or concept, create four questions that are relevant to the benefit or experience being offered. For each question, provide four answers in simple English. These answers should be short descriptive statements that reflect what the person using this product would say they "want" or "think is important."
                To summarize:
                You know the product or concept based on the study name and description provided above.
...
"""
            ...
            status.update(label="📋 Generating Final Thoughts...")
            final_thoughts_prompt = (
                f"Write a closing statement for a research study about the  {self.name}s  here is the list of {self.name} = {self.data}'.\n\n"
                f"Product description:\n{description}\n\n"
                f"The participant has just completed questions about their needs, lifestyle, mindset, and relationship to the product.\n\n"
                "Write 2–3 short paragraphs that:\n"
                "- Thank the participant genuinely for their time and insights\n"
    ...
    def _step2_questions(self, product):
        study_data = st.session_state.agent_study_data
        st.info(f"Generating questions for: {study_data['study_name']}")
        ...
        with col2:
            if st.button("Next: Prelim Questions →"):
                st.session_state.agent_study_step = 3
                st.rerun()

    def _step3_prelim_questions(self):
        study_data = st.session_state.agent_study_data
        prelim_questions = study_data["prelim_questions"]
        st.info("Edit preliminary questions (e.g., demographic questions)")
        ...
        with col1:
            if st.button("← Back to Questions"):
                st.session_state.agent_study_step = 2
                st.rerun()
        with col3:
            if st.button("Review Study →"):
                st.session_state.agent_study_step = 4
