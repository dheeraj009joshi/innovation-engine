...
PROMPT = PromptTemplate(
 ...
 Your task is to process the situations from the uploaded document and group them into a set of mutually exclusive, radically different high-level themes and, within each theme, a set of mutually exclusive, highly specific subthemes.
Definitions & Instructions:
Theme (Definition):
 A theme is a high-level, unambiguous, and mutually exclusive “bucket” that describes a single, coherent type of situation. Each theme must be meaningfully different from all others. The theme’s label and description must clearly capture the core concepts shared by all situations within it.
...
Subtheme (Definition):
 A subtheme is a specific, mutually exclusive facet within a theme, grouping situations that share a distinct characteristic or angle. Each subtheme must be clear, concrete, and limited in scope so that each situation fits only one subtheme within its parent theme.
 Do not use vague, generic, or “miscellaneous” categories such as “General Habits,” “Miscellaneous,” or “Other.” Each subtheme must have a clear, descriptive label that precisely captures what all its situations have in common.
...
Resonance (Definition):
 A situation “resonates with” or “fits” a theme/subtheme if all the main words and concepts in the situation are explicitly covered by the theme’s and subtheme’s label and description, with no ambiguity and no other fit possible.
Instructions:
For each situation in the uploaded document, extract the Situation Name and Situation Description fields exactly as written (no changes).


Assign each situation to exactly one theme and one subtheme (both mutually exclusive).
...
For the Situation References column, provide the count of evidence snippets used for that situation.


Do not invent, summarize, merge, or paraphrase the situation names or descriptions—only reword the example consumer statement for clarity.


If any subtheme contains more than 12 situations, you MUST split it further into smaller, more specific, and descriptive subthemes. Avoid lumping situations into a single subtheme unless they are truly, specifically related.
...
For each situation within that subtheme, provide:
Situation Name (exact, from document)


Situation Description (exact, from document)
...
Situation References (count of evidence snippets in the "Evidence_Snippets" column)
