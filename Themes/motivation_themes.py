...
Your task is to process the Motivations from the uploaded document and group them into a set of mutually exclusive, radically different high-level themes and, within each theme, a set of mutually exclusive, highly specific subthemes.

Definitions & Instructions:

Theme (Definition):
A theme is a high-level, unambiguous, and mutually exclusive “bucket” that describes a single, coherent type of Motivation. Each theme must be meaningfully different from all others. The theme’s label and description must clearly capture the core concepts shared by all Motivations within it.
...
Subtheme (Definition):
A subtheme is a specific, mutually exclusive facet within a theme, grouping Motivations that share a distinct characteristic or angle. Each subtheme must be clear, concrete, and limited in scope so that each Motivation fits only one subtheme within its parent theme.
Do not use vague, generic, or “miscellaneous” categories such as “General Habits,” “Miscellaneous,” or “Other.” Each subtheme must have a clear, descriptive label that precisely captures what all its Motivations have in common.
...
A Motivation “resonates with” or “fits” a theme/subtheme if all the main words and concepts in the Motivation are explicitly covered by the theme’s and subtheme’s label and description, with no ambiguity and no other fit possible.

Instructions:

- For each Motivation in the uploaded document, extract the Motivation Name and Motivation Description fields exactly as written (no changes).
- Assign each Motivation to exactly one theme and one subtheme (both mutually exclusive).
- Each Theme must contain exactly 4 Motivations.
...
- For the Motivation References column, provide the count of evidence snippets used for that Motivation.
- Do not invent, summarize, merge, or paraphrase the Motivation names or descriptions—only reword the example consumer statement for clarity.
- If any subtheme contains more than 12 Motivations, you MUST split it further into smaller, more specific, and descriptive subthemes. Avoid lumping Motivations into a single subtheme unless they are truly, specifically related.
...
For each Motivation within that subtheme, provide:
- Motivation Name (exact, from document)
- Motivation Description (exact, from document)
...
- Motivation References (count of evidence snippets in the "Evidence_Snippets" column)

Special Quality Rule:
If you find yourself about to create a subtheme with a generic or vague label (e.g., “General,” “Miscellaneous,” “Other,” “General Habits & Environment”), STOP and break the Motivations into more sharply differentiated, specific subthemes. Each subtheme must be clearly understandable and stand alone on its own label and description.
